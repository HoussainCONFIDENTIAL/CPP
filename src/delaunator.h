...
// monotonically increases with real angle, but doesn't need expensive trigonometry
    ...
    Delaunator::Delaunator(std::vector<double> const& in_coords)
        ...
        //// Déterminer les coordonnées minimales et maximales du vecteur ////
        double max_x = std::numeric_limits<double>::min();
        double max_y = std::numeric_limits<double>::min();
        double min_x = std::numeric_limits<double>::max();
        double min_y = std::numeric_limits<double>::max();
        std::vector<std::size_t> ids;   //// indices des points triés par rapport à la distance au triangle central i
        ids.reserve(n);
        ...
        std::size_t i0 = INVALID_INDEX;     //// seed point -> proche du centre (cx, cy)
        std::size_t i1 = INVALID_INDEX;     //// point le plus proche de i0
        std::size_t i2 = INVALID_INDEX;     //// point qui forme le plus petit triangle avec les 2 autres
    ...
    /// retourne un truc bizarre par rapport aux pseudos-angles et la taille du hash
